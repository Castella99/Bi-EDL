...
def load_CARZero(
    ...
    if not multi:
        ...
        CARZero_model = builder.build_CARZero_dqn_wo_self_atten_gl_mlp_multi_model(cfg).to(device)
    ...
    with torch.no_grad():
        ...
        for i in range(bs):
            ...
            if not multi:
                ...
                # (1, S_txt+1, D) * (B, S_txt, D)


            t2i_cls = t2i_cls.squeeze(-1)  ## use text as query, use image as k, v, so image batch size have not distrubed the result
            # # 
        ...
        # cls = i2t_cls
        # i2t_SimR_bs = torch.cat(i2t_SimR_bs, dim=0)
        # t2i_SimR_bs = torch.cat(t2i_SimR_bs, dim=1)
        # np.save('/haoranlai/Project/CARzero/padchest_inference/it2_SimR.npy', i2t_SimR_bs.detach().cpu().numpy())
...
def get_mcq_dqn_similarities(CARZero_model, imgs, txts, similarity_type="both", dropout=False, feature=False, atten_map=False, mode='both', torch_tensor=False, seperate=False, ts=False):
     # warnings
    if similarity_type not in ["global", "local", "both"]:
        raise RuntimeError(
            f"similarity type should be one of ['global', 'local', 'both']"
    ...
    for i in range(bs):
        ...
        if mode == 'i2t' :
            ...
            if ts:
                i2t_cls = i2t_cls / (torch.exp(CARZero_model.i2t_tau))
                t2i_cls = t2i_cls / (torch.exp(CARZero_model.t2i_tau))
            cls = CARZero_model.alpha * i2t_cls + (1 - CARZero_model.alpha) * t2i_cls

        if torch_tensor:
            cls_bs.append(cls.unsqueeze(0))
        else:
    ...
    if torch_tensor:
        return cls
    if seperate:
        i2t_cls = torch.cat(i2t_cls_bs, dim=0) # (N, B)
        t2i_cls = torch.cat(t2i_cls_bs, dim=0) #  (N, B)
        return i2t_cls.numpy(), t2i_cls.numpy()
...
def get_attention_maps(CARZero_model, imgs, txts):
    """Load a CARZero pretrained classification model
    ...
    -------
    attention_maps :
        attention maps between each imgs and text
    """
    caption_ids = []
    attention_mask = []
    token_type_ids = []
    for cls_name, txts in txts.items():
    ...
    txts = {"caption_ids": caption_ids, "attention_mask": attention_mask, "token_type_ids":token_type_ids}
    
    with torch.no_grad():
        img_emb_l, img_emb_g = CARZero_model.image_encoder_forward(imgs)
        text_emb_l, text_emb_g, sents = CARZero_model.text_encoder_forward(
            txts["caption_ids"], txts["attention_mask"], txts["token_type_ids"]
        )
        attention_maps = CARZero_model.get_attn_maps(img_emb_l, text_emb_l, sents)

    return attention_maps, sents
...
def dqn_shot_classification(CARZero_model, imgs, cls_txt_mapping, dropout=False, feature=False, atten_map=False, mode=None, multi=False, mcq=False, torch_tensor=False, seperate=False, ts=False):
    ...
    if mcq :
        if feature :
            return get_mcq_dqn_similarities(
            CARZero_model, imgs, text_batch, similarity_type="both", dropout=dropout, feature=feature, mode=mode)
        elif atten_map :
            return get_mcq_dqn_similarities(
            CARZero_model, imgs, text_batch, similarity_type="both", dropout=dropout, atten_map=atten_map, mode=mode)
        else :
            return get_mcq_dqn_similarities(
            CARZero_model, imgs, text_batch, similarity_type="both", dropout=dropout, mode=mode, torch_tensor=torch_tensor, seperate=seperate, ts=ts)
...
def retrieval_shot_classification(CARZero_model, imgs, cls_txt_mapping, text_bs=256):
    """Load a CARZero pretrained classification model
