...
def build_CARZero_dqn_wo_self_atten_gl_mlp_multi_model(cfg):
    CARZero_model = models.CARZero_model_dqn_wo_self_atten_gl_mlp_multi.CARZeroDQNWOSAGLMLP(cfg)
    return CARZero_model
...
def build_dqn_wo_self_atten_mlp_module(cfg, average_attn_weights=True):
    fusion = models.dqn_wo_self_atten_mlp.TQN_Model(cfg, average_attn_weights=average_attn_weights)
    return fusion


def build_dqn_wo_self_atten_mlp_orig_module(cfg):
    fusion = models.dqn_wo_self_atten_mlp.TQN_Orig_Model(cfg)
    return fusion
