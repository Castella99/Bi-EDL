...
def main(args) :
    ...
    pn_i2t_features_umap = [UMAP(n_components=2, random_state=42).fit_transform(np.concatenate((p_i2t_feat[:,i,:], n_i2t_feat[:,i,:]), axis=0)) for i in tqdm(range(n_logit.shape[1]))]
    pn_t2i_features_umap = [UMAP(n_components=2, random_state=42).fit_transform(np.concatenate((p_t2i_feat[:,i,:], n_t2i_feat[:,i,:]), axis=0)) for i in tqdm(range(n_logit.shape[1]))]

    pn_i2t_features_umap = np.array(pn_i2t_features_umap)
    pn_t2i_features_umap = np.array(pn_t2i_features_umap)

    p_i2t_feat_umap = pn_i2t_features_umap[:, :p_i2t_feat.shape[0], :]
    p_t2i_feat_umap = pn_t2i_features_umap[:, :p_t2i_feat.shape[0], :]
    n_i2t_feat_umap = pn_i2t_features_umap[:, p_i2t_feat.shape[0]:, :]
    n_t2i_feat_umap = pn_t2i_features_umap[:, p_t2i_feat.shape[0]:, :]
    
    np.save(f"{save_dir}/pn_i2t_features_umap.npy", pn_i2t_features_umap)
    np.save(f"{save_dir}/pn_t2i_features_umap.npy", pn_t2i_features_umap)
    ...
    for i in range(n_logit.shape[1]):
        ...
        plt.subplot(4, 4, i+1)
        plt.scatter(p_i2t_feat_umap[i][false_sample, 0], p_i2t_feat_umap[i][false_sample, 1], s=0.5, c='red', label='Positive - False', alpha=0.5)
        plt.scatter(n_i2t_feat_umap[i][false_sample, 0], n_i2t_feat_umap[i][false_sample, 1], s=0.5, c='green', label='Negative - False', alpha=0.5)
        plt.scatter(p_i2t_feat_umap[i][true_sample, 0], p_i2t_feat_umap[i][true_sample, 1], s=1, c='blue', label='Positive - True', alpha=0.5)
        plt.scatter(n_i2t_feat_umap[i][true_sample, 0], n_i2t_feat_umap[i][true_sample, 1], s=1, c='orange', label='Negative - True', alpha=0.5)
        plt.xlabel('UMAP Component 1')
        plt.title(f"Class - {class_name[i]}")
        plt.legend()
        plt.axis('off')   
    plt.tight_layout()
    plt.savefig(f"{save_dir}/NIH_i2t_umap_plot.png")
    plt.show()
    ...
    for i in range(n_logit.shape[1]):
        ...
        false_sample = (test_df.iloc[:,i+3]==0)
        plt.subplot(4, 4, i+1)
        plt.scatter(p_t2i_feat_umap[i][false_sample, 0], p_t2i_feat_umap[i][false_sample, 1], s=0.5, c='red', label='Positive - False', alpha=0.5)
        plt.scatter(n_t2i_feat_umap[i][false_sample, 0], n_t2i_feat_umap[i][false_sample, 1], s=0.5, c='green', label='Negative - False', alpha=0.5)
        plt.scatter(p_t2i_feat_umap[i][true_sample, 0], p_t2i_feat_umap[i][true_sample, 1], s=1, c='blue', label='Positive - True', alpha=0.5)
        plt.scatter(n_t2i_feat_umap[i][true_sample, 0], n_t2i_feat_umap[i][true_sample, 1], s=1, c='orange', label='Negative - True', alpha=0.5)
        plt.xlabel('UMAP Component 1')
        plt.title(f"Class - {class_name[i]}")
        plt.legend()
        plt.axis('off')   
    plt.tight_layout()
    plt.savefig(f"{save_dir}/NIH_t2i_umap_plot.png")
    plt.show()
