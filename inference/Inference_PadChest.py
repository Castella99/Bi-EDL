...
class_name = ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Pneumonia', 'No Finding']

positive = {"0": ["There is Atelectasis"], "1": ["There is Cardiomegaly"], "2": ["There is Consolidation"], "3": ["There is Edema"], "4": ["There is Pneumonia"], "5" : ["There is no Finding"]}
negative = {"0": ["There is no Atelectasis"], "1": ["There is no Cardiomegaly"], "2": ["There is no Consolidation"], "3": ["There is no Edema"], "4": ["There is no Pneumonia"]}

prompts = ["There is Atelectasis", "There is Cardiomegaly", "There is Consolidation", "There is Edema", "There is Pneumonia", "There is no Finding", "There is no Atelectasis", "There is no Cardiomegaly", "There is no Consolidation", "There is no Edema", "There is no Pneumonia"]
...
def main(args) :
    ...
    data_path = "/shared/home/mai/Taehun/Uncertainty/data/PadChest/images"
    image_csv = pd.read_csv(os.path.join("PadChest", 'padchest_multi_label_image.csv'), )
    image_csv['Path'] = image_csv['Path'].apply(lambda x: os.path.join(data_path, x.split('/')[-1]))

    with open(os.path.join("PadChest", "manual_image.json"), 'r') as f:
    ...
    labels = [label_json[k] for k in key ]
    ...
    gt = encoded_labels[:, test_query_index]
    ...
    pos_results.to_csv(os.path.join(save_dir, "PadChest_pos_results.csv"))
    neg_results.to_csv(os.path.join(save_dir, "PadChest_neg_results.csv"))
    pnc_results.to_csv(os.path.join(save_dir, "PadChest_pnc_results.csv"))
    ...
    if args.dir :
        ...
        pos_i2t_results.to_csv(os.path.join(save_dir, "PadChest_pos_i2t_results.csv"))
        neg_i2t_results.to_csv(os.path.join(save_dir, "PadChest_neg_i2t_results.csv"))
        pnc_i2t_results.to_csv(os.path.join(save_dir, "PadChest_pnc_i2t_results.csv"))
        
        pos_t2i_results.to_csv(os.path.join(save_dir, "PadChest_pos_t2i_results.csv"))
        neg_t2i_results.to_csv(os.path.join(save_dir, "PadChest_neg_t2i_results.csv"))
        pnc_t2i_results.to_csv(os.path.join(save_dir, "PadChest_pnc_t2i_results.csv")) 
